...
public final class AccuracyHitboxCornerHeuristic extends ClassicHeuristic<AccuracyHitboxCornerHeuristic.PerfectAttackMeta> {
  public AccuracyHitboxCornerHeuristic(Heuristics parentCheck) { super(parentCheck, HeuristicsClassicType.ATTACK_ACCURACY, PerfectAttackMeta.class); }
  ...
  public void evaluateFightAccuracy(ProtocolPacketEvent event) {
    ...
    if (attackedEntity != null && !attackedEntity.moving(0.05)) return;
  ...
  @PacketSubscription(priority = ListenerPriority.HIGH, packetsIn = {POSITION_LOOK, LOOK})
  public void receiveMovement(ProtocolPacketEvent event) {
    ...
    PerfectAttackMeta heuristicMeta = metaOf(user);
    if (!attackData.recentlyAttacked(1000) || attackData.recentlySwitchedEntity(500) || attackData.lastReach() < 1.0) return;
    ...
    if (heuristicMeta.distanceToPerfectYawList.size() > 20) {
      ...
      if (failRate < 5 && (yawSpeedAverage > 10 || distanceAverage > 10)) {
        heuristicMeta.vl++;
        String description = "maintains high attack accuracy whilst aiming at hitbox corners (fail:" + MathHelper.formatDouble(failRate, 2) + "%, r:" + MathHelper.formatDouble(yawSpeedAverage, 2) + ", d:" + MathHelper.formatDouble(distanceAverage, 2) + ") vl:" + MathHelper.formatDouble(heuristicMeta.vl, 2);
        flag(player, description);
        if (heuristicMeta.vl >= 2) user.nerf(AttackNerfStrategy.DMG_MEDIUM, "13");
  ...
  private double averageOf(List<? extends Number> data) {
    double sum = 0;
    for (Number element : data) sum += element.doubleValue();
    return sum == 0 ? 0 : sum / data.size();
  }
