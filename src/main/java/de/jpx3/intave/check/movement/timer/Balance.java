...
public final class Balance extends MetaCheckPart<Timer, Balance.BalanceMeta> {
  ...
  @PacketSubscription(packetsOut = RESPAWN)
  ...
  public void receiveMovement(ProtocolPacketEvent event) {
    ...
    if (highToleranceMode || meta.abilities().probablyFlying()) allowedLagInMilliseconds = Integer.MAX_VALUE;
    if (System.currentTimeMillis() - timerData.lastRespawn < 6000) allowedLagInMilliseconds = Math.max(allowedLagInMilliseconds, 8000);
    ...
    if (timerData.timerBalance > overflowLimit && !movementData.isInVehicle()) {
      ...
      double displayValue = Math.max(0.01, TimeUnit.NANOSECONDS.toMillis(timerData.timerBalance) / 50d);
      statisticApply(user, CheckStatistics::increaseFails);
      Violation violation = Violation.builderFor(Timer.class).forPlayer(player)
        .withMessage("moved too frequently").withDetails(formatDouble(displayValue, 2) + " ticks ahead")
      ...
      if (timerData.timerBalance > 0) timerData.timerBalance -= 750_000L;
      if (System.currentTimeMillis() - timerData.lastTimerFlag > 10000) decrementer.decrement(user, 0.01);
    }
  }
  ...
  public static class BalanceMeta extends CheckCustomMetadata {
    public long timerBalance = Long.MIN_VALUE / 2;
