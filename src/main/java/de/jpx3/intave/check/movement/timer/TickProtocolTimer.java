...
public final class TickProtocolTimer extends MetaCheckPart<Timer, TickProtocolTimer.Meta> {
  ...
  public TickProtocolTimer(Timer parentCheck) { super(parentCheck, Meta.class); }
  ...
  public void playerTick(ProtocolPacketEvent event) {
    ...
    if (positionBearing) {
      ...
      if (!user.meta().protocol().sendsClientTickEnd()) sampleNegativeClock(user, meta);
    }
    ...
    if (PacketTypes.isClientEndTick(type)) {
      ...
      } else meta.tickBuffer = Math.max(0.0D, meta.tickBuffer - 0.2D);
      if (meta.positionSeenThisTick) sampleNegativeClock(user, meta);
  ...
  @PacketSubscription(priority = LOWEST, packetsIn = {VEHICLE_MOVE, STEER_VEHICLE}, ignoreCancelled = false)
  public void vehicleTick(ProtocolPacketEvent event) {
    ...
    if (meta.vehicleClock > now + VEHICLE_DRIFT_NS) {
      ...
      if (meta.vehicleBuffer >= 2.0D) {
        flag(user, "vehicle packets are running ahead of the client clock", "ahead=" + ((meta.vehicleClock - now) / 1_000_000L) + "ms", 2.0D);
        meta.vehicleBuffer = 1.0D;
      }
      meta.vehicleClock -= CLIENT_TICK_NS;
    } else {
  ...
  private void sampleNegativeClock(User user, Meta meta) {
    MovementMetadata movement = user.meta().movement();
    long now = System.nanoTime();
    if (movement.awaitTeleport || movement.expectTeleport || movement.isInVehicle() || user.meta().abilities().ignoringMovementPackets() || !stable(user)) {
      resetNegative(meta);
      return;
    }
    ...
    if (behind > NEGATIVE_DRIFT_NS) {
      ...
      if (meta.negativeBuffer >= 2.0D) {
        flag(user, "movement packets are running behind the client clock", "behind=" + (behind / 1_000_000L) + "ms", 1.5D);
        meta.negativeBuffer = 1.0D;
      }
      meta.negativeClock += CLIENT_TICK_NS;
    } else meta.negativeBuffer = Math.max(0.0D, meta.negativeBuffer - 0.10D);
  }

  private static boolean isPositionBearing(PacketTypeCommon type) {
    return type == PacketType.Play.Client.PLAYER_POSITION || type == PacketType.Play.Client.PLAYER_POSITION_AND_ROTATION;
  }
  ...
  private void flag(User user, String message, String details, double vl) {
    Violation violation = Violation.builderFor(Timer.class).forPlayer(user.player()).withCheckName("Timer").withMessage(message).withDetails(details).withVL(vl).build();
