...
public final class FarPlaceEnvelope extends MetaCheckPart<PlacementAnalysis, FarPlaceEnvelope.Meta> {
  ...
  public void receive(ProtocolPacketEvent event) {
    ...
    if (user.meta().movement().isInVehicle() || user.meta().abilities().hasViewEntity) return;
    ...
    try {
      ...
      for (double eye : eyeHeights) {
        minSq = Math.min(minSq, distanceSqToUnitBlock(movement.positionX, movement.positionY + eye, movement.positionZ, block));
        minSq = Math.min(minSq, distanceSqToUnitBlock(movement.lastPositionX, movement.lastPositionY + eye, movement.lastPositionZ, block));
      }
      ...
      meta.buffer += 1.0D;
      if (meta.buffer < 2.0D) return;
      Violation violation = Violation.builderFor(PlacementAnalysis.class)
        .forPlayer(user.player()).withCheckName("Scaffold").withMessage(COMMON_FLAG_MESSAGE)
  ...
  private static double blockReachDistance(User user) {
    ...
    if (!user.meta().protocol().supportsInteractionRangeAttributes()) return fallback;
    double tracked = user.meta().abilities().attributeValue("player.block_interaction_range");
    if (!Double.isFinite(tracked) || tracked <= 0.0D) return fallback;
    if (creative && Math.abs(tracked - 4.5D) < 1.0E-6D) return fallback;
    return Math.min(tracked, 64.0D);
  }

  private static double distanceSqToUnitBlock(double x, double y, double z, BlockPosition block) {
    double minX = block.getX(), minY = block.getY(), minZ = block.getZ();
